...
* **Verification:** Direct API calls (bypassing browser automation).
...
2.  **Verification (Direct API):** For 67 N Parsonage St, 33 Cedar Heights, and the discovered comps, the CLI uses direct ASP API endpoints to query Dutchess County ParcelAccess. It extracts the official 2026 assessed value, SBL number, and physical characteristics.
...
The Gemini CLI needs the servers mapped in its configuration file. The setup script outputs the JSON block to append for the Filesystem and SQLite servers.
